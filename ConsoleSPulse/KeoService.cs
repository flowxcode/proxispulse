...
namespace ConsoleSPulse
{
    public class KeoService
    {
        public byte[] InitProtocol(IProxiLAB keo)
        {
            byte[] answer = null;
            uint error = 0;
        ...
        public byte[] SendTcl(IProxiLAB keo, byte[] apdu, out long elapsedMs)
        {
            uint error = 0;
        ...
        public byte[] SendTclTearing(IProxiLAB keo, byte[] apdu, uint cycles, out long elapsedMs)
        {
            ...
            error = keo.Spulse.LoadSpulseCsvFile(filepath, cycles, (uint)eFrameTypeFormat.FRAME_TYPE_SPULSE, (uint)eEmulatorLoadSpulseMode.STAND_ALONE);
            ...
            else
            {
                ...
                returnBuffer = new byte[rxBufferLength];
                for (uint i = 0; i < rxBufferLength; i++)
                    returnBuffer[i] = rxBuffer[i];
            }
